...
import { fetchPreviousLaunches } from '../actions'; // Adjust the import path as necessary
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faWikipediaW, faYoutube, faReddit } from '@fortawesome/free-brands-svg-icons';

function PreviousLaunch() {
  const dispatch = useDispatch();
  const { loading, data, error } = useSelector(state => state.previousLaunches);
  ...
  if (!data.length) {
    return <div>No previous launches</div>;
  }

  const launch = data[data.length-1]; // Assuming you want to display the most recent previous launch
